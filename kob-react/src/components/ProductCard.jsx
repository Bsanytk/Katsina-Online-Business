...
export default function ProductCard({ product, onEdit, onDelete, onBuyClick }) {
  ...
  const getImagesArray = () => {
    if (product.images && Array.isArray(product.images) && product.images.length > 0) return product.images.map(img => typeof img === 'string' ? img : img.url || img.preview || imagePlaceholder)
    const single = product.imageURL || product.imageUrl || product.mainImage
    return single ? [single] : [imagePlaceholder]
  }

  const images = getImagesArray()
  const displayImage = images[currentImageIndex] || imagePlaceholder
  ...
  const whatsappLink = product.whatsappNumber ? `https://wa.me/${product.whatsappNumber}?text=${whatsappMessage}` : `https://wa.me/?text=${whatsappMessage}`
  ...
  return (
    <article className="group bg-white rounded-xl shadow-sm hover:shadow-xl transition-all duration-300 border border-neutral-200 overflow-hidden flex flex-col h-full animate-fade-in relative">
      ...
      <div className="relative w-full h-52 bg-neutral-100 overflow-hidden">
        <img
          src={imageError ? imagePlaceholder : displayImage}
        ...
        {images.length > 1 && (
          <>
            <button
              ...
              className="absolute left-2 top-1/2 -translate-y-1/2 bg-white/80 hover:bg-white text-neutral-700 rounded-full p-1 shadow-md transition-all text-sm"
              aria-label="Previous Image"
            >‹</button>
            <button
              onClick={handleNextImage}
              className="absolute right-2 top-1/2 -translate-y-1/2 bg-white/80 hover:bg-white text-neutral-700 rounded-full p-1 shadow-md transition-all text-sm"
              aria-label="Next Image"
            >›</button>
          </>
        )}

        {/* Draft/Live & KOB Express Badges */}
      ...
      <div className="p-4 flex flex-col flex-grow">
        <h3 className="text-md font-bold text-neutral-800 line-clamp-1 mb-1 capitalize">{product.title || product.name}</h3>
        ...
        <p className="text-xs text-neutral-500 mb-3 line-clamp-2 flex-grow">{product.description}</p>
        ...
        <div className="flex gap-2">
          {canManage ? (
            <div className="flex gap-2 w-full">
              <button onClick={() => onEdit?.(product)} className="flex-1 px-3 py-2 bg-neutral-100 hover:bg-neutral-200 text-neutral-700 rounded-lg font-bold transition-colors text-xs border border-neutral-300">
                Edit
              </button>
              <button onClick={() => onDelete?.(product)} className="px-3 py-2 bg-red-50 hover:bg-red-100 text-red-600 rounded-lg font-bold transition-colors text-xs border border-red-200">
                Delete
              </button>
            </div>
          ) : (
            <div className="flex gap-2 w-full">
              <button onClick={() => onBuyClick?.(product)} className="flex-1 px-3 py-2 bg-kob-primary hover:bg-kob-primary-dark text-white rounded-lg font-bold transition-all text-xs">
                Details
              </button>
              <a href={whatsappLink} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center p-2 bg-green-500 hover:bg-green-600 text-white rounded-lg transition-all" title="Chat on WhatsApp">
