...
export default function ConversationList({ onSelectConversation }) {
  ...
  if (error) {
    return (
      <Card
        variant="outlined"
        className="p-6 rounded-lg bg-red-50 border-red-200"
      >
      ...
      {conversations.map((conversation) => (
        <Card
          ...
          className={`p-4 rounded-lg cursor-pointer transition-all ${
            selectedId === conversation.id
              ? "bg-kob-light border-kob-primary"
              : "hover:bg-gray-50"
          }`}
        ...
        >
          <div className="flex items-start justify-between mb-2">
            ...
            {/* Unread Badge */}
            {(conversation.unreadCounts?.[user?.uid] || 0) > 0 && (
              <div className="bg-red-500 text-white text-xs font-bold rounded-full w-6 h-6 flex items-center justify-center flex-shrink-0">
                {conversation.unreadCounts[user.uid] > 99
                  ? "99+"
