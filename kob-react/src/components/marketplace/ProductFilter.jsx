...
export default function ProductFilter({
  ...
  useEffect(() => {
    ...
    if (
      ...
      selectedCategory !== t("productFilter.allCategories")
    ) {
    ...
    if (selectedSellerType !== "all") {
      if (selectedSellerType === "verified") {
        filtered = filtered.filter(
          (p) => p.isVerified === true || p.verified === true
        );
      } else if (
    ...
    if (priceRange !== "all") {
      filtered = filtered.filter((p) => {
        ...
        if (priceRange === "20000-100000")
          return price > 20000 && price <= 100000;
