...
export default function ProductForm({
  ...
  function validateForm() {
    ...
    if (!formData.sellerIDNumber.trim())
      errors.sellerIDNumber = "KOB ID is required";
  ...
  if (!isVerified && !isEditMode) {
    return (
      <Card
        variant="outlined"
        className="p-10 text-center border-2 border-amber-200 bg-amber-50 rounded-2xl shadow-sm"
      >
        ...
        <p className="text-amber-700 mb-8 max-w-md mx-auto leading-relaxed">
          Sorry! Only <strong>verified sellers</strong> can post products on KOB
          Marketplace.
        </p>
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          <a
            ...
            className="px-8 py-3 bg-green-600 text-white rounded-xl font-bold shadow-lg"
          >
            Chat with Admin
          </a>
          <button
            ...
            className="px-8 py-3 bg-kob-primary text-white rounded-xl font-bold shadow-lg"
          >
            Verification Form
          </button>
        </div>
      ...
      <div className="space-y-6">
        ...
        >
          <label className="flex items-center gap-3 cursor-pointer">
            <input
              ...
              onChange={() =>
                setFormData((p) => ({ ...p, isDraft: !p.isDraft }))
              }
              className="w-5 h-5 rounded accent-kob-primary"
            />
            <span
              className={`font-bold ${
                formData.isDraft ? "text-amber-900" : "text-green-900"
              }`}
            >
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <Textarea
            ...
            placeholder="Size, Color, Quality..."
          />
          ...
          <div className="space-y-2">
            <label className="block text-sm font-semibold text-kob-dark">
              Delivery Method
            </label>
            <div className="flex gap-4 p-4 border rounded-xl bg-gray-50 border-gray-200">
              <label className="flex items-center gap-2 cursor-pointer font-bold text-green-700">
                <input
                  ...
                  checked={formData.deliveryOption === "KOB Express Delivery"}
                  onChange={handleChange}
              ...
              <label className="flex items-center gap-2 cursor-pointer text-gray-600">
                <input
                  ...
                  name="deliveryOption"
                  value="Others"
                  checked={formData.deliveryOption === "Others"}
                  onChange={handleChange}
