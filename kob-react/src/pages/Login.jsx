/**
 * Login.jsx — KOB Marketplace Login Page
 *
 * FIXED v3:
 * ✅ loginUser called correctly — from useAuth() context
 * ✅ FCM init non-blocking — fire-and-forget after routing
 * ✅ window.location.href for hard redirect — forces ProfileContext refresh
 * ✅ ForgotPasswordModal preserved exactly
          ...
          <div className="p-6">
            {success ? (
              <div className="text-center py-4">
                <div
                  className="w-14 h-14 bg-emerald-50 rounded-full
...
export default function Login() {
  ...
  // ✅ loginUser comes from AuthContext — always defined
  ...
  async function handleSubmit(e) {
    ...
    try {
      // ✅ Step 1: Login — loginUser is correct (email, password)
      const userCredential = await loginUser(email, password);

      // ✅ Step 2: FCM — fully non-blocking fire-and-forget
      // Route FIRST, then init FCM in background
      // FCM failure must never block navigation
      if (userCredential?.user?.uid) {
        import("../services/fcm")
          .then(({ initFCM }) => initFCM(userCredential.user.uid))
          .catch((fcmErr) =>
            console.warn("[KOB Login] FCM background init:", fcmErr.message)
          );
      }

      // ✅ Step 3: Hard redirect — forces ProfileContext to reload
      // window.location.href is intentional here (not navigate())
          ...
          <div className="p-7">
            ...
            {error && (
              <div
                ...
                rounded-xl mb-5"
              >
                <span className="text-red-500 flex-shrink-0 text-sm mt-0.5">
                  ⚠
                </span>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <button
                ...
                className="w-full flex items-center justify-center
                  gap-2 py-3.5 mt-2 bg-[#4B3621] text-white
